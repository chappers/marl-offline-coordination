...
class DoubleDQNTrainer(DQNTrainer):
    ...
    def train_from_torch(self, batch):
        ...
        for batch in range(batch_num):
            ...
            best_action_idxs_ = best_action_idx.max(-1, keepdim=True)[1]
            best_action_idxs.append(best_action_idxs_.permute(0, 2, 1))
            obs_qs.append(obs_q)
        ...
        with torch.no_grad():
            for batch in range(batch_num):
                ...
                target_q_values_ = target_q_value.max(-1, keepdim=True)[1]
                target_q_values.append(target_q_values_.permute(0, 2, 1))
        ...
        y_pred = torch.sum(
            obs_qs * torch.from_numpy(np.stack(actions, 0)).float(),
            dim=-1,
            keepdim=True,
        )
