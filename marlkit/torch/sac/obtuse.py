...
class OBTTrainer(TorchTrainer):
    ...
    def calculate_convergence(self, np_batch):
        """
        Used to calculate convergence condition
        """
        ...
        # calculate the divergence between vae_pos and vae_neg
        # and return...

        metric = 0
        with torch.no_grad():
            sampled_actions_neg, raw_sampled_actions_neg = self.vae_neg.decode_multiple(
                obs
            )
            sampled_actions_pos, raw_sampled_actions_pos = self.vae_pos.decode_multiple(
                obs
            )
    ...
    def train_from_torch(self, batch):
        ...
        if self.kernel_choice == "laplacian":
            mmd_neg_loss = self.mmd_loss_laplacian(
                raw_sampled_actions_neg, new_obs_raw_actions, sigma=self.mmd_sigma
            )
            mmd_pos_loss = self.mmd_loss_laplacian(
                raw_sampled_actions_pos, new_obs_raw_actions, sigma=self.mmd_sigma
            )
            vae_loss = self.mmd_loss_laplacian(
                raw_sampled_actions_pos,
                raw_sampled_actions_neg,
                sigma=self.mmd_sigma,
            )
            mmd_behavior_pen = torch.tanh(vae_loss)
        elif self.kernel_choice == "gaussian":
            mmd_neg_loss = self.mmd_loss_gaussian(
                raw_sampled_actions_neg, new_obs_raw_actions, sigma=self.mmd_sigma
            )
            mmd_pos_loss = self.mmd_loss_gaussian(
                raw_sampled_actions_pos, new_obs_raw_actions, sigma=self.mmd_sigma
            )
            vae_loss = self.mmd_loss_gaussian(
                raw_sampled_actions_pos,
                raw_sampled_actions_neg,
                sigma=self.mmd_sigma,
            )
            mmd_behavior_pen = torch.tanh(mmd_loss)
        ...
        if self.mode == "auto":
            mmd_loss = (
                self.log_alpha.exp()
                * mmd_behavior_pen
                * ((mmd_neg_loss - mmd_pos_loss))
            )
        else:
            mmd_loss = 100 * mmd_behavior_pen * ((mmd_neg_loss - mmd_pos_loss))

        policy_loss = (mmd_loss + alpha * log_pi - q_new_actions).mean()
        ...
        if self._need_to_update_eval_statistics:
            ...
            self.eval_statistics["VAE Positive"] = np.mean(ptu.get_numpy(mmd_pos_loss))
            self.eval_statistics["VAE Negative"] = np.mean(ptu.get_numpy(mmd_neg_loss))
            self.eval_statistics["VAE Convergence"] = np.mean(ptu.get_numpy(vae_loss))
