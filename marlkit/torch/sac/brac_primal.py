"""
This implements BRAC? using the primal form KL divergence
The primal form is fairly trivial - just change MMD to KL divergence,
we ignore the gradient penalty update for simplicity
"""
...
class BEARTrainer(TorchTrainer):
    ...
    def div_kl_loss(self, samples1, samples2, *args, **kwargs):
        # allow symmetry
        out = (nn.KLDivLoss(samples1, samples2) + nn.KLDivLoss(samples2, samples1)) / 2
        return out
    ...
    def train_from_torch(self, batch):
        ...
        actor_samples, _, _, _, _, _, _, raw_actor_actions = self.policy(
            obs.unsqueeze(1).repeat(1, self.num_samples_mmd_match, 1).view(-1, obs.shape[1]),
            return_log_prob=True,
        )
        ...
        if self.kernel_choice == "laplacian":
            div_loss = self.mmd_loss_laplacian(raw_sampled_actions, raw_actor_actions, sigma=self.mmd_sigma)
        elif self.kernel_choice == "gaussian":
            div_loss = self.mmd_loss_gaussian(raw_sampled_actions, raw_actor_actions, sigma=self.mmd_sigma)
        elif self.kernel_choice == "kl":
            div_loss = self.div_kl_loss(raw_sampled_actions, raw_actor_actions)
        ...
        if self._n_train_steps_total >= 40000:
            ...
            if self.mode == "auto":
                policy_loss = (-policy_loss + self.log_alpha.exp() * (div_loss - self.target_mmd_thresh)).mean()
            else:
                policy_loss = (-policy_loss + 100 * div_loss).mean()
        else:
            if self.mode == "auto":
                policy_loss = (self.log_alpha.exp() * (div_loss - self.target_mmd_thresh)).mean()
            else:
                policy_loss = 100 * div_loss.mean()
        ...
        if self._need_to_update_eval_statistics:
            ...
            self.eval_statistics.update(create_stats_ordered_dict("MMD Loss", ptu.get_numpy(div_loss)))
            self.eval_statistics.update(
