...
class AWACTrainer(TorchTrainer):
    ...
    def run_bc_batch(self, replay_buffer, policy):
        ...
        policy_logpp = self.policy.log_prob(og, u)
    ...
    def pretrain_policy_with_bc(
        ...
        for i in range(steps):
            (
                ...
                train_mse_loss,
                train_stats,
            ) = self.run_bc_batch(train_buffer, policy)
            ...
            (
                ...
                test_mse_loss,
                test_stats,
            ) = self.run_bc_batch(test_buffer, policy)
    ...
    def test_from_torch(self, batch):
        ...
        # new_log_pi = next_dist.log_prob(new_next_actions)
    ...
    def train_from_torch(
        ...
        policy_logpp = self.policy.log_prob(obs, u)
        ...
        if (
            not pretrain
            and self.buffer_policy_reset_period > 0
            and self._n_train_steps_total % self.buffer_policy_reset_period == 0
        ):
            ...
            for i in range(self.num_buffer_policy_train_steps_on_reset):
                if self.train_bc_on_rl_buffer:
                    if self.advantage_weighted_buffer_loss:
                        buffer_dist = self.buffer_policy(obs)
                        buffer_u = actions
                        buffer_new_obs_actions = buffer_dist.rsample()
        ...
        if self._need_to_update_eval_statistics:
            ...
            if self.train_bc_on_rl_buffer:
                ...
                _, buffer_test_logp_loss, _, _ = self.run_bc_batch(
                    self.replay_buffer.validation_replay_buffer, self.buffer_policy
                )
                buffer_dist = self.buffer_policy(obs)
                kldiv = torch.distributions.kl.kl_divergence(dist, buffer_dist)
