    ...
    def train_from_torch(self, batch):
        ...
        with torch.no_grad():
            ...
            state_rep = (
                ...
                .repeat(1, 10, 1)
                .view(next_obs.shape[0] * 10, next_obs.shape[1])
            )
            ...
            target_Q = 0.75 * torch.min(target_qf1, target_qf2) + 0.25 * torch.max(
        ...
        actor_samples, _, _, _, _, _, _, raw_actor_actions = self.policy(
            obs.unsqueeze(1)
            .repeat(1, self.num_samples_mmd_match, 1)
            .view(-1, obs.shape[1]),
            return_log_prob=True,
        )
        actor_samples = actor_samples.view(
