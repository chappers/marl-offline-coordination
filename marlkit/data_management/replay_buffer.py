...
class FullMAReplayBuffer(object, metaclass=abc.ABCMeta):
    """
    A class used to save and replay data.
    """
    ...
    def add_path(self, path):
        ...
        For marl, the path is the agents obs, which needs ot be added in its entirity for trainign
        """
        # a sample is the WHOLE path
        self.add_sample(
            observation=path["observations"],
            states=path["states"],
            states_0=path["states_0"],
            active_agents=path["active_agents"],
            action=path["actions"],
            reward=path["rewards"],
            next_observation=path["next_observations"],
            # next_states=path["next_states"],
            # next_states_0=path["next_states_0"],
            terminal=path["terminals"],
            agent_info=path["agent_infos"],
